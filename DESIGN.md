...
ClipTen 像一张随手可取的系统记忆便签：它栖身于菜单栏，在首次启动、再次打开应用或用户点开图标时展开，不占 Dock、不制造独立窗口，也不把短暂的剪贴板动作包装成复杂工作流。视觉表达完全服从 macOS 原生菜单，让应用看起来像系统本来就有的一项能力。
...
每条记录以单行预览出现：连续空白与换行折叠为空格，默认最多显示 72 个字符，超出后以省略号收束；完整文本保留在系统工具提示中。当前可访问名称为“复制：”加最多 140 个字符的折叠预览，不提供完整长文本。系统菜单负责宽度、边距、行高和屏幕边缘避让，不设置自定义断点或固定面板尺寸。
...
- **启动监控边界：** 当前在历史与缩略图恢复完成后才记录剪贴板基线，恢复期间的新复制可能漏记。目标应区分启动前的旧内容与启动后的新复制，不能通过延后基线把两者一并忽略。
- **展开菜单的异步状态：** `history.onChange` 当前只更新状态栏反馈，不重建已展开的菜单。恢复期间通过再次打开应用进入的菜单可能停留在加载态，需关闭再打开；应补充加载完成后的菜单刷新及真实菜单跟踪验收。
...
- **Do** 让完整文本继续通过工具提示可用，并为菜单项提供带“复制”动作的可访问预览；不要把截断的可访问名称描述为完整正文。
